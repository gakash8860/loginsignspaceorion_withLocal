...
class _TempAccessFloorPageState extends State<TempAccessFloorPage> {
  ...
  TempAccessFloor floorWeb;
  List<TempAccessFlat> flat;
  List<TempAccessFlat> flatWeb;
  List<TempAccessRoom> room;
  ...
  Future getFlatForTempUserWeb() async {
    await getTokenWeb();
    final url =
        API+'getallflatbyonlyflooridf_id/?f_id=' + floorWeb.fId.toString();
    // String token = 'ec21799a656ff17d2008d531d0be922963f54378';
    final response = await http.get(url, headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
    ...
    if (response.statusCode > 0) {
      ...
      if(response.statusCode==200){
        List<dynamic> data = jsonDecode(response.body);
        setState(() {
          flatWeb = data.map((data) => TempAccessFlat.fromJson(data)).toList();
        });
  ...
  Future getRoomForTempUserWeb() async {
    await getTokenWeb();
    final url = API+'getallroomsbyonlyflooridf_id/?flt_id=' + flatWeb[0].fltId;
    final response = await http.get(url, headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
      'Authorization': 'Token $tokenWeb',
  ...
  deviceContainerWebData(String dId, int index) async {
