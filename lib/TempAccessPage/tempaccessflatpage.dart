...
class _TempAccessFlatPageState extends State<TempAccessFlatPage> {
  ...
  Future getRoomForTempUserWeb() async {
      await getTokenWeb();
    final url = API+'getallroomsbyonlyflooridf_id/?flt_id=' + flatWeb.fltId.toString();
    final response = await http.get(url, headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
      'Authorization': 'Token $tokenWeb',
  ...
  getDataWeb(String dId) async {
    final String url = API+'getpostdevicePinStatus/?d_id=' +dId.toString();
    await getTokenWeb();
    http.Response response = await http.get(url, headers: {
      'Content-Type': 'application/json; charset=UTF-8',
      'Authorization': 'Token $tokenWeb',
  ...
  Future getPinsNameWeb(String dId) async {
    final url = API+"editpinnames/?d_id=" + dId;
    await getTokenWeb();
    // try {
    final response = await http.get(Uri.parse(url), headers: {
      'Content-Type': 'application/json',
...
// server response
  ...
  deviceContainerDataWeb(String dId, int index) async {
    ...
    print('catchReturn123Webdataflat ${catchReturn}');
    print('catchReturn123Webdataflat ${dId}');
    // var sensorData=
    responseGetDataWeb = [
      widget.switch1_get = catchReturn["pin1Status"],
      widget.switch2_get = catchReturn["pin2Status"],
                ...
                children: List.generate(
                  // 9,
                    responseGetDataWeb.length - 3,
                        (index) {
                      print('Something ${responseGetDataWeb.length}');
                      print('Something ${namesDataListWeb[index]}');
                      print('Something ${namesDataListWeb[index]}');
                    ...
                    slivers: <Widget>[
                      ...
                      SliverList(
                        delegate: SliverChildBuilderDelegate((context, index) {
                          print('asdfirstasasasas ${dvWeb.length}');
                          if (index <dvWeb.length) {

                            print('asdf ${dvWeb.length}');
                            Text(
                              "Loading",
                            ...
                            return Container(
                              child: Column(
                                children: [
                                  deviceContainerWeb(dvWeb[index].dId, index),
                                  Container(
                                    //
                                    // color: Colors.green,
                                      height: 35,
