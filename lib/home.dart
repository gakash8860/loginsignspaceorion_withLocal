...
import 'TemporaryUser/showTempUser.dart';
import 'BillPrediction.dart';
import 'ProfilePage.dart';
import 'Setting_Page.dart';

import 'package:shared_preferences/shared_preferences.dart';
import 'about_Genorion.dart';
import 'package:loginsignspaceorion/ProfilePage.dart';
import 'components/constant.dart';
import 'googleAssistant/DeviceApps.dart';
...
class HomePage extends StatefulWidget {
  ...
  HomePage({Key key, this.pt, this.fl, @required this.rm, this.dv})
      : super(key: key);
...
// pt.pType=;
  ...
  Future<bool> getPinsName()async{
    ...
    String url="http://genorionofficial.herokuapp.com/editpinnames/?d_id=DIDM12932021AAAAAD";
    var response;
    try{
      response= await http.get(Uri.parse(url),headers: {
        'Content-Type': 'application/json',
...
// pt.pType=;
                            ...
                            if (placeData.contains('empty')) {
                              ...
                              return Container(
                                ...
                                child: DropdownButtonFormField<String>(
                                  ...
                                  items: placeData.map((selectedPlace) {
                                    int i = 0;
                                    while (i < placeData.length) {
                                      i++;
                                    }
                    ...
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (
                              ...
                              Container(
                                child: HomePage(pt: pt, fl: fl, rm: rm, dv: dv),
                              )),
                    );
                  },
                ),
  ...
  @override
  void initState() {
    // databaseHelper.initiateDatabase().then((value) {
    //   print('initialize');
    //   // loadData();
    // });
    placeVal = fetchplace();
    ...
    // floorval2=fetchOnlyFloor(pt.pId);
    print('toString $floorval2');
    // print('Ptype---->  ${widget.pt.pType}');
    openPlaceBox();
    ...
    timer = Timer.periodic(Duration(seconds: 2), (timer) async {
      ...
      // print('Catch Return InitState--> ${catchReturn.toString()}');
      // getData(catchReturn);
      // checkDeviceData();
      await getDevices(tabbarState);
      await getDatafunc2();
    ...
    print('RoomResponse-->  $roomResponse');
    // tabbarState = roomResponse;

    // Timer.periodic(Duration(seconds: 1), (timer) {
    //   for(int i=0;i<widget.rm.length;i++){
    //     print('loop  ${widget.rm[i].rId}');
    //     print('Room Length  ${widget.rm.length}');
    //     // tabbarState=rm[0].rId;
    //     // print('TabbarState Loop-> ${tabbarState}');
    //     getDevices(rm[i].rId);
    //   }
    //
    // });
    ...
    tabC.addListener(() {
      print('Roomsssss RID-->>>>>>>   ${rm[tabC.index].rId}');
      setState(() {
        // tabbarState = rm[0].rId;
        tabbarState = rm[tabC.index].rId;
      });
    });
    print("InitTabbar-->$tabbarState");
    super.initState();
    ...
    return Scaffold(
      ...
      bottomNavigationBar: SingleChildScrollView(
        child: BottomNavyBar(
          ...
          //       icon: Icon(Icons.mic), label: '', backgroundColor: Colors.purple),
          //   BottomNavigationBarItem(
          //     icon: Icon(Icons.add),
          //     label: 'Add Device',
